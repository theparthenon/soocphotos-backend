...
from api.auto_album import generate_event_albums, regenerate_event_titles
from api.mixins.list_view_mixin import ListViewSet
from api.mixins.pagination_mixin import StandardResultsSetPagination
from api.models import AlbumAuto, Face, Person, Photos
    ...
    def get_queryset(self):
        ...
        return (
            AlbumAuto.objects.prefetch_related(
                Prefetch("owner"),
                Prefetch("photos", queryset=Photos.visible.all()),
...
class AlbumAutoListViewSet(ListViewSet):
    ...
    def get_queryset(self):
        cover_photo_query = Photos.objects.filter(hidden=False)
        return (
