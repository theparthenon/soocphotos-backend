...
def get_current_job():
    job_detail = None
    running_job = Job.objects.filter(finished=False).order_by("-started_at").first()
...
def get_search_term_examples(user):
    ...
    possible_ids = list(
        Photos.objects.filter(owner=user)
...
def get_server_stats():
    ...
    for user in User.objects.filter(~Q(id=get_deleted_user().id)):
        ...
        # Number of Photos
        number_of_photos = Photos.objects.filter(Q(owner=user)).count()
        # Number of Videos
        number_of_videos = Photos.objects.filter(Q(owner=user) & Q(video=True)).count()
        # Number of Files (To-Do: Owner missing)
        # number_of_files = File.objects.filter(Q(owner=user)).count()
        # Number of Captions
        number_of_captions = Photos.objects.filter(
            Q(owner=user) & Q(captions_json__user_caption__isnull=False)
        ).count()
        # Number of Generated Captions
        number_of_generated_captions = Photos.objects.filter(
            Q(owner=user) & Q(captions_json__im2txt__isnull=False)
        ).count()
        # Most common file type for photos (To-Do: Add mime type)
        # most_common_file_type = Photos.objects.filter(Q(owner=user)).values("file_extension").annotate(count=Count("file_extension")).order_by("-count").first()
        ...
        # Number of Favorites
        number_of_favorites = Photos.objects.filter(
            Q(owner=user) & Q(rating__gte=user.favorite_min_rating)
        ).count()
        # Number of Hidden
        number_of_hidden = Photos.objects.filter(Q(owner=user) & Q(hidden=True)).count()
        # Total File Size (To-Do: Should be files)
        total_file_size = (
            Photos.objects.filter(Q(owner=user)).aggregate(Sum("size"))["size__sum"]
            or None
        )
        user = {
...
def get_count_stats(user):
    num_photos = Photos.visible.filter(Q(owner=user)).distinct().count()
    num_missing_photos = Photos.objects.filter(
