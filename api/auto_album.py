...
def regenerate_event_titles(user, job_id):
    if Job.objects.filter(job_id=job_id).exists():
        ...
        lrj = Job.objects.create(
            ...
            job_type=Job.JOB_GENERATE_AUTO_ALBUM_TITLES,
        )
        lrj.save()
    try:
        ...
        lrj.save()
        logger.info("job %d: updated lrj entry to db", job_id)
...
def generate_event_albums(user, job_id):
    if Job.objects.filter(job_id=job_id).exists():
        ...
        lrj = Job.objects.create(
            ...
            queued_at=datetime.now().replace(tzinfo=pytz.utc),
            started_at=datetime.now().replace(tzinfo=pytz.utc),
            job_type=Job.JOB_GENERATE_AUTO_ALBUMS,
        )
        lrj.save()

    try:
        photos = (
            Photos.objects.filter(Q(owner=user))
        ...
        logger.info(
            "job %d: made %d groups out of %d images", job_id, target_count, len(photos)
        )
        ...
        for idx, group in enumerate(groups):
            ...
            if len(group) >= 2:
                ...
                if qs.count() == 1:
                    album = qs.first()
                    logger.info("job %d: update auto album %s", job_id, album.id)
