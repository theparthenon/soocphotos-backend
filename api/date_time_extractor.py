...
"""Module for extracting date and time from a string."""
...
def _regexp_group_range(a, b):
    """Return a regular expression group range."""
...
def _extract_no_tz_datetime_from_str(x, regexp=REGEXP_NO_TZ, group_mapping=None):
    ...
    try:
        ...
        if delta.days > 30:
            logger.error(
                "Error while parsing datetime from '%s': Parsed datetime is %s days in the future.",
                x,
                delta.days,
            )
            return None
        ...
        logger.error(
            "Error while trying to create datetime using '%s': datetime arguments %s. Regexp used: '%s'",
            x,
            datetime_args,
            regexp,
        )
...
class TimeExtractionRule:
    ...
    should be one of the following:
      ...
      - "server_local" - the timezone of the librephotos server - not very useful since we run
        docker containers in UTC timezone.
      - "user_default" - user default timezone
      - "name:<timezone_name>" - the timezone with the name <timezone_name>
    If either "source_tz" or "report_tz" could not be obtained the rule is considered to be not
    applicable.
    ...
    to only the photos/videos that meet the condition's requirement. Supported conditions:
      - "condition_path": "<regexp>" - rule only applied to files with full path
                                       (as seen by backend) matching the regexp
      - "condition_filename": "<regexp>" - rule only applied to files with filename matching the
                                           regexp
      - "condition_exif": "<tag_name>//<regexp>" - first "//" is considered end of tag name and
                                                   the rule is only applied if value of tag
                                                   <tag_name> exists and matches the regexp.

    If multiple conditions are provided the rule is considered applicable only if all of them
    are met.
    ...
    def get_required_exif_tags(self):
        """Gets the required exif tags and appends them to the returned result."""

        condition_tag, pattern = (  # pylint: disable=unused-variable
    ...
    def apply(
        ...
        """
        Applies the rule to the given path, EXIF tags, GPS coordinates, user default timezone,
        and user defined timestamp.
        Args:
            path (str): The path of the file.
            exif_tags (dict): The EXIF tags of the file.
            gps_lat (float): The latitude of the GPS coordinates.
            gps_lon (float): The longitude of the GPS coordinates.
            user_default_tz (str): The user's default timezone.
            user_defined_timestamp (datetime): The user-defined timestamp.
        Returns:
            datetime or None: The applied timestamp based on the rule type. Returns None if the
...
def set_as_default_rule(rule):
    """Sets the given rule as the default rule."""
...
def set_as_other_rule(rule):
    """Sets the given rule as not the default rule."""
...
def as_rules(configs):
    """Returns a list of `TimeExtractionRule` objects from the given JSON string."""
...
def extract_local_date_time(
    ...
    """
    Extracts local date and time based on the given rules and parameters.

    Args:
        path: The path information.
        rules: The list of rules for extracting local date and time.
        exif_getter: The function to get exif values.
        gps_lat: The latitude for GPS location.
        gps_lon: The longitude for GPS location.
        user_default_tz: The user's default timezone.
        user_defined_timestamp: The user-defined timestamp.

    Returns:
        The extracted local date and time if successful, None otherwise.
    """
